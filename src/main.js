...
const { MessageTooLargeError, InvalidTokenError, InvalidChannelIdError, ChordDBNotStartedError } = require("./errors")
...
class UDB {
    ...
    async start() {
        ...
        if (user_info.id) {
            console.log(`\x1b[32m chorddb: connected as ${user_info.username}\x1b[0m`);
        } else {
        ...
        if (channel_info.id) {
            console.log(`\x1b[32m chorddb: linked to channel ${channel_info.name}\x1b[0m`);
        } else {
    ...
    async write(data) {
        ...
        const r = await dc_call(`/channels/${this.ch_id}/messages`, "POST", {"content": enc_data});
